...
echo -e "${GREEN}=== K2Pay Multi-Architecture Package Builder ===${NC}"
...
for ARCH in "${ARCHS[@]}"; do
    ...
    if go build \
        ...
        PKG_DIR=$(mktemp -d)
        PKG_NAME="k2pay-${PKGVER}-1-${ARCH}.pkg.tar.zst"
        ...
        install -Dm644 "$SCRIPT_DIR/k2pay.service" "$PKG_DIR/usr/lib/systemd/system/k2pay.service"
        ...
        [ -f LICENSE ] && install -Dm644 LICENSE "$PKG_DIR/usr/share/licenses/k2pay/LICENSE"
...
pkgbase = k2pay
pkgver = ${PKGVER}-1
pkgdesc = K2Pay - A lightweight payment gateway supporting USDT, WeChat and Alipay
url = https://github.com/HenZenKuriRIP/k2pay
builddate = $(date +%s)
...
license = MIT
depend = glibc
backup = etc/k2pay/config.yaml
EOF

        # Create .INSTALL file (copy from k2pay.install)
        if [ -f "$SCRIPT_DIR/k2pay.install" ]; then
            cp "$SCRIPT_DIR/k2pay.install" "$PKG_DIR/.INSTALL"
        fi
