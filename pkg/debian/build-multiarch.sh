...
echo -e "${GREEN}=== K2Pay Debian Multi-Architecture Package Builder ===${NC}"
...
rm -rf debian/k2pay debian/.debhelper debian/files debian/debhelper* debian/*.log debian/*.substvars 2>/dev/null || true
...
for ARCH in "${ARCHS[@]}"; do
    ...
    if go build \
        ...
        # Install binary
        install -Dm755 "k2pay-$ARCH" "$PKG_DIR/usr/bin/k2pay"

        # Install config
        install -Dm644 config.yaml "$PKG_DIR/etc/k2pay/config.yaml"

        # Install web files
        install -dm755 "$PKG_DIR/usr/share/k2pay/web"
        cp -r web/templates "$PKG_DIR/usr/share/k2pay/web/" 2>/dev/null || echo "  Warning: templates not found"
        cp -r web/static "$PKG_DIR/usr/share/k2pay/web/" 2>/dev/null || echo "  Warning: static not found"

        # Install database migration scripts
        install -dm755 "$PKG_DIR/usr/share/k2pay/db"
        cp -r db/* "$PKG_DIR/usr/share/k2pay/db/" 2>/dev/null || echo "  Warning: db directory not found"

        # Install systemd service
        install -Dm644 "$SCRIPT_DIR/k2pay.service" "$PKG_DIR/lib/systemd/system/k2pay.service"

        # Create directories
        install -dm750 "$PKG_DIR/var/lib/k2pay"
        install -dm750 "$PKG_DIR/var/lib/k2pay/uploads"
        install -dm750 "$PKG_DIR/var/lib/k2pay/qrcode"
        install -dm750 "$PKG_DIR/var/log/k2pay"

        # Install docs
        mkdir -p "$PKG_DIR/usr/share/doc/k2pay"
        [ -f README.md ] && install -Dm644 README.md "$PKG_DIR/usr/share/doc/k2pay/README.md"
...
Depends: libc6 (>= 2.17)
Description: K2Pay - A lightweight payment gateway
...
EOF
        ...
        if [ -f "$SCRIPT_DIR/conffiles" ]; then
            ...
            echo "/etc/k2pay/config.yaml" > "$PKG_DIR/DEBIAN/conffiles"
        fi

        # Build package
        echo -e "${YELLOW}Building .deb package...${NC}"
        PACKAGE_NAME="k2pay_${VERSION}_${ARCH}.deb"
...
echo "  sudo dpkg -i $RELEASE_DIR/k2pay_${VERSION}_amd64.deb"
echo "  sudo apt-get install -f"
echo ""
echo -e "${BLUE}To install (arm64):${NC}"
echo "  sudo dpkg -i $RELEASE_DIR/k2pay_${VERSION}_arm64.deb"
echo "  sudo apt-get install -f"
echo ""
