...
echo -e "${GREEN}=== K2Pay Tarball Multi-Platform Package Builder ===${NC}"
...
for PLATFORM in "${!PLATFORMS[@]}"; do
    ...
    if CGO_ENABLED=0 GOOS=$GOOS GOARCH=$GOARCH go build \
        ...
        PKG_NAME="k2pay-${VERSION}-${PLATFORM}"
...
# K2Pay Installation Guide for $PLATFORM
...
4. Run the application:
   Double-click k2pay.exe or run from Command Prompt:
   k2pay.exe
...
EOF
            ...
            cat >> "$PKG_DIR/INSTALL-${PLATFORM}.txt" << EOF
   tar -xzf k2pay-${VERSION}-${PLATFORM}.tar.gz
   cd k2pay-${VERSION}-${PLATFORM}
