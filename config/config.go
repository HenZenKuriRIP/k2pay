...
func Load() (*Config, error) {
	...
	if p := os.Getenv("K2PAY_CONFIG"); p != "" {
		...
		viper.AddConfigPath("/etc/k2pay") // 生产 install.sh
	}
...
func getDefaultDataDir() string {
	switch runtime.GOOS {
	case "linux":
		return "/var/lib/k2pay"
	case "windows":
		// Windows: 当前目录/k2pay_data
		return filepath.Join(getExeDir(), "k2pay_data")
	default:
		// macOS 和其他: 当前目录/k2pay_data
		return filepath.Join(getExeDir(), "k2pay_data")
	}
}
...
func createDefaultConfig() error {
	...
	configContent := fmt.Sprintf(`# K2Pay 配置文件
# 所有配置项都可在此文件中设置
