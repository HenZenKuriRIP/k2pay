...
type Order struct {
	...
	TxHash         string          `gorm:"type:varchar(100)" json:"tx_hash"`            // 交易哈希 / api_trade_no
	ApiTradeNo     string          `gorm:"type:varchar(100)" json:"api_trade_no"`       // 易支付兼容：上游/链上流水号
	Buyer          string          `gorm:"type:varchar(100)" json:"buyer"`              // 易支付兼容：付款人标识
	...
	RefundMoney    decimal.Decimal `gorm:"type:decimal(18,6);default:0" json:"refund_money"` // 已退款金额(订单原币)
	...
	Channel        string          `gorm:"type:varchar(20);default:'local'" json:"channel"` // 支付通道: local, epay
