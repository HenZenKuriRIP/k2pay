...
func (s *NotifyService) sendNotify(notifyURL string, params map[string]string) bool {
	// SSRF 防护：拒绝内网/元数据等目标
	if err := util.ValidateCallbackURL(notifyURL); err != nil {
		log.Printf("sendNotify blocked unsafe notify URL: %v", err)
		return false
	}
	...
	// 使用安全客户端：Dial 阶段拦截内网 IP，并限制重定向
	client := util.SafeHTTPClient(30 * time.Second)
