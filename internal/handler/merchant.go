...
func (h *MerchantHandler) Login(c *gin.Context) {
	...
	if err := model.DB.Where("p_id = ? AND status = 1", req.PID).First(&merchant).Error; err != nil {
		c.JSON(http.StatusOK, gin.H{"code": -1, "msg": "商户号或密码错误"})
		return
	}

	// 验证密码
	if merchant.Password == "" || !util.CheckPassword(req.Password, merchant.Password) {
	...
	// 弱口令登录后强制改密
	mustChange := merchant.MustChangePassword || util.IsWeakPassword(req.Password)
	if mustChange && !merchant.MustChangePassword {
		model.DB.Model(&merchant).Update("must_change_password", true)
	...
	c.JSON(http.StatusOK, gin.H{
		"code":                 1,
		"msg":                  "登录成功",
		"must_change_password": merchant.MustChangePassword,
		"data": gin.H{
			"token":                tokenString,
			"must_change_password": merchant.MustChangePassword,
			"merchant": gin.H{
...
func (h *MerchantHandler) UpdateProfile(c *gin.Context) {
	...
	if req.NotifyURL != "" {
		if err := util.ValidateCallbackURL(req.NotifyURL); err != nil {
			c.JSON(http.StatusOK, gin.H{"code": -1, "msg": "notify_url 无效: " + err.Error()})
			return
		}
	...
	if req.ReturnURL != "" {
		if err := util.ValidateCallbackURL(req.ReturnURL); err != nil {
			c.JSON(http.StatusOK, gin.H{"code": -1, "msg": "return_url 无效: " + err.Error()})
			return
		}
...
func (h *MerchantHandler) ChangePassword(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusOK, gin.H{"code": -1, "msg": "参数错误，密码至少8位"})
		return
	}

	if err := util.ValidateNewPassword(req.NewPassword); err != nil {
		c.JSON(http.StatusOK, gin.H{"code": -1, "msg": err.Error()})
		return
	}

	if req.NewPassword == req.OldPassword {
		c.JSON(http.StatusOK, gin.H{"code": -1, "msg": "新密码不能与旧密码相同"})
		return
	}
...
func (h *MerchantHandler) ConfirmPayment(c *gin.Context) {
	...
	result := model.DB.Model(&order).
		Where("status IN ?", []model.OrderStatus{model.OrderStatusPending, model.OrderStatusExpired}).
		Updates(updates)
	if result.Error != nil {
		c.JSON(http.StatusOK, gin.H{"code": -1, "msg": "确认失败: " + result.Error.Error()})
		return
	}
	if result.RowsAffected == 0 {
		c.JSON(http.StatusOK, gin.H{"code": -1, "msg": "订单状态已变更，请刷新后重试"})
		return
	}
	...
	if err := service.GetOrderService().CreditMerchantBalance(&order); err != nil {
		log.Printf("Merchant confirm order %s credit balance failed: %v", tradeNo, err)
		c.JSON(http.StatusOK, gin.H{"code": -1, "msg": "订单已确认，但入账失败，请联系管理员: " + err.Error()})
		return
	}
...
// GetMonitorConfig 获取商户对接配置（网关地址 + 密钥二维码 + App 信息）
