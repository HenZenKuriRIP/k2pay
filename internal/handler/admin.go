...
func (h *AdminHandler) Login(c *gin.Context) {
	...
	// 弱口令登录后强制改密
	mustChange := admin.MustChangePassword || util.IsWeakPassword(req.Password)
	if mustChange && !admin.MustChangePassword {
		model.GetDB().Model(&admin).Update("must_change_password", true)
...
func (h *AdminHandler) CreateMerchant(c *gin.Context) {
	...
	// 设置密码
	if req.Password != "" {
		if err := util.ValidateNewPassword(req.Password); err != nil {
			c.JSON(http.StatusOK, gin.H{"code": -1, "msg": err.Error()})
			return
		}
		...
		// 未设密码时标记强制改密，管理员后续设置或商户首次登录前需设密
...
func (h *AdminHandler) ChangePassword(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusOK, gin.H{"code": -1, "msg": "参数错误，新密码至少8位"})
		return
	}
	...
	if err := model.GetDB().Model(&admin).Updates(map[string]interface{}{
...
func (h *AdminHandler) UploadAppVersion(c *gin.Context) {
	...
	// 生成文件名
	filename := fmt.Sprintf("k2pay_v%s_%d.apk", versionName, time.Now().Unix())
