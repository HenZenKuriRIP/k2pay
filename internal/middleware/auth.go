	...
	return func(c *gin.Context) {
		...
		if adminID, ok := c.Get("admin_id"); ok {
			if !strings.HasSuffix(c.Request.URL.Path, "/password") {
				...
				if err := model.GetDB().Select("id", "must_change_password").First(&admin, adminID).Error; err == nil && admin.MustChangePassword {
					c.JSON(http.StatusForbidden, gin.H{
						"code":                 -2,
						"msg":                  "请先修改默认/弱密码后再使用系统",
...
func MerchantAuth(cfg *config.Config) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// 强制改密：除修改密码接口外一律拒绝
		if !strings.HasSuffix(c.Request.URL.Path, "/password") && merchant.MustChangePassword {
			c.JSON(http.StatusForbidden, gin.H{
				"code":                 -2,
				"msg":                  "请先修改默认/弱密码后再使用系统",
				"must_change_password": true,
